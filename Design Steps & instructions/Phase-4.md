...
For `Name` type `SUNDAY-WORDPRESS-CONTENT`  or choose your preferred name.
...
for `Throughput mode` pick `bursting` which links performance to how much space you consume. The more consumed, the higher performance. The other option Provisioned allows for performance to be specified independently of consumption.  
Untick `Enable encryption of data at rest` .. in production you would leave this on, but for this demo which focuses on architecture, it simplifies the implementation.  
Click `Next`
...
In the `Virtual Private Cloud (VPC)` dropdown select `Sunday-VPC`  
You should see 3 rows.  
Make sure `us-east-1a`, `us-east-1b` & `us-east-1c` are selected in each row.  
In the `us-east-1a` row, select `sn-App-A` in the subnet ID dropdown, and in the security groups dropdown select `Sunday-VPC-SGEFS` & remove the default security group  
In the `us-east-1b` row, select `sn-App-B` in the subnet ID dropdown, and in the security groups dropdown select `Sunday-VPC-SGEFS` & remove the default security group  
In the `us-east-1c` row, select `sn-App-C` in the subnet ID dropdown, and in the security groups dropdown select `Sunday-VPC-SGEFS` & remove the default security group  
...
# Phase 4B - Add an fsid to the parameter store
...
Under `Name` enter `/Sunday/Wordpress/EFSFSID` 
...
# Phase 4C - Connect the file system to the EC2 instance & copy data
...
```
EFSFSID=$(aws ssm get-parameters --region us-east-1 --names /Sunday/Wordpress/EFSFSID --query Parameters[0].Value)
...
# Phase 4D - Test that the wordpress app can load the media
...
# Phase 4E - Update the launch template with the config to automate the EFS part
...
for `Template version description` enter `App only, uses EFS filesystem defined in /Sunday/Wordpress/EFSFSID`  
